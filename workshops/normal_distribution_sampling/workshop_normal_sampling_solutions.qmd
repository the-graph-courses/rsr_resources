...
But is it *unusually* low? To answer this, we need to know what kind of sample means we would get just by random chance if the hospital were really no different from the nation. That "no difference" claim is the **null hypothesis** ($H_0$): the hospital's mean birth weight equals the national mean.
...
ggplot(sim_data_8, aes(x = mean)) +
  ...
  geom_vline(xintercept = mean_flint_small, color = "#3ea387", linetype = "dashed") +
  coord_cartesian(xlim = c(2.8, 3.9)) +
  labs(title = "Sampling distribution (n=8) with line at our sample mean", x = "Mean (kg)", y = "Count")
```
...
ggplot(sim_data_30, aes(x = mean)) +
  ...
  geom_vline(xintercept = mean_flint_large, color = "#3ea387", linetype = "dashed") +
  coord_cartesian(xlim = c(2.8, 3.9)) +
  labs(title = "Sampling distribution (n=30) with line at our sample mean", x = "Mean (kg)", y = "Count")
```
...
Finally, we use `pt()` to turn the t-score into a p-value. This gives us the same kind of answer as the simulation: the probability of getting a sample mean this low (or lower) **if $H_0$ is true**. For a t-distribution, we also need the degrees of freedom, which is $n - 1$.
...
**Ans:** The null hypothesis is that the hospital's mean birth weight equals the national mean ($H_0$: $\bar{x}_\text{hospital} = \mu_\text{national}$). The t-statistic tells us how many standard errors the hospital mean lies below the national mean. The p-value is the probability of seeing a sample mean this low (or lower) if $H_0$ were true.

Because it is small (about 0.014), a mean this low would be unlikely under the null, which is evidence against $H_0$ in our exercise.
:::
