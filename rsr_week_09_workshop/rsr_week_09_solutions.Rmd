...
**Answer:** Purpose 1, estimating a causal or adjusted exposure effect.
...
**Answer:** Purpose 3, comparing predictors after mutual adjustment. A coefficient table with adjusted estimates is the main output. A forest plot can also help compare coefficients visually.
...
```{r load-packages}
if (!require(pacman)) install.packages("pacman")
pacman::p_load(readxl, tidyverse, here, janitor, broom, gt, gtsummary, performance, see, sandwich, lmtest)
pacman::p_load_gh("the-graph-courses/inspectdf")
```
...
**Answer:** Before labeling, all variables are numeric. `depression_score`, `chronological_age`, and `oslo_social_support` look roughly symmetric. `sex`, `grade_level`, and `residence` are categorical codes and should be labeled.
...
**Answer:** In the simple model, each 1-year increase in chronological age is associated with about **0.45 points higher** PHQ-9 score on average. The 95% CI is about **0.18 to 0.71**, so it excludes 0.
...
**Answer:** Holding chronological age, grade level, residence, and Oslo social support constant, males score about **1.04 points lower** on the PHQ-9 than females, on average.
...
**Answer:** The age coefficient shrank from about **0.45** to about **0.15**, and the 95% CI changed from excluding 0 to including 0. In the simple model, age looked clearly associated with depression score; in the multiple model, age no longer has a clear unique association after grade and the other predictors are included.
...
**Answer:** The adjusted 95% CIs exclude 0 for sex (`Male` versus `Female`), Grade 12 versus Grade 9, rural residence versus urban residence, and Oslo social support. Chronological age, Grade 10, and Grade 11 have adjusted intervals that include 0.
...
**Answer:** The model has **8** parameters: the intercept plus seven predictor terms.
...
**Answer:** Yes. With **546** observations and **8** parameters, we have about **68** observations per parameter, well above the rough rule of 10.
...
**Answer:** It mainly threatens **U**, uncorrelated errors. Students in the same school may have correlated residuals because they share school context. That is a study-design issue, not a normality issue.
...
**Answer:** Chronological age and grade level show the highest collinearity. This matches the age-by-grade table. The VIF values are around 2.7 to 2.8, so the overlap is real but not severe by common teaching thresholds such as 5 or 10.
...
**Answer:** Yes. The HC3 robust results keep the same main conclusions: sex, Grade 12, rural residence, and Oslo social support remain clearly associated with PHQ-9 score; chronological age remains unclear after adjustment. This makes the heteroscedasticity less damaging for the substantive story, though it should still be mentioned.
