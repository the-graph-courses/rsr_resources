...
Curated teaching datasets for ordinary least squares. **27** resources are downloaded (or pulled from permissive R packages): **25** numbered cleaning pipelines with tidy `snake_case` CSVs **plus** the OpenIntro NHANES adult-500 extract (original column names) **and** AIS athletes exported from `DAAG`. Each gets a quick-view thumbnail.
...
**Short version for a public, non-commercial, teaching-oriented package on GitHub:** all 27 are usable. For a more general-purpose package (CRAN-compatible, any licence), you'd want to:
