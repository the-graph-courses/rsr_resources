...
# =============================================================================
# FIG 1b: same people, binary 0/1 outcome (jittered)
# =============================================================================
set.seed(11)
f_binary <- ggplot(klosa, aes(age, weak_grip)) +
  geom_jitter(height = 0.04, width = 0.35, colour = teal, alpha = .18, size = 1.35) +
  scale_y_continuous(breaks = c(0, 1), labels = c("0  (not weak)", "1  (weak)")) +
  scale_x_continuous(breaks = seq(45, 90, 10)) +
  coord_cartesian(xlim = c(45, 91), ylim = c(-0.15, 1.15)) +
  labs(x = "age (years)", y = "weak grip",
       title = "Same men, now coded as 0 or 1") + base
save_fig(f_binary, "fig_binary_scatter.png", w = 8.0, h = 5.0)
