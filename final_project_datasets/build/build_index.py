...
ORDER = [
    "serbia_antibiotics", "sciatica_primary_care",
    ...
    "tibial_bone_strength", "nigeria_malaria_households",
    "ageing_qol_europe", "essens_adolescents",
]

# These are public data extracts, not datasets deposited alongside one research paper.
# Empty since nhanes_adults and us_births_2014 were withdrawn; every remaining
# dataset is deposited alongside a paper.
NON_PAPER_SOURCES = set()
...
EXCLUDED_OUTCOMES = {
    "ggt_arterial_stiffness": {
        # Still shipped as a column -- withdrawn only as an outcome.
        "abi_max": "no clinical variance in this cohort: 0 of 876 participants fall "
                   "below the 0.90 PAD threshold, 94.7% sit in the normal 1.00-1.30 "
                   "band, and there are 41 distinct values on a 0.01 recording grid. "
                   "It is uncorrelated with pwv_max (r = 0.01) and every association "
                   "runs in the clinically wrong direction (higher BMI, alcohol and "
                   "exercise all predict higher ABI), most plausibly a cuff artefact "
                   "in thicker limbs. Its diagnostics are the cleanest of the four "
                   "outcomes, which is what makes it misleading",
    },
    "peru_child_anaemia": {
    ...
    "bangladesh_growth_monitoring": {
        ...
        # The four service-utilisation and growth-chart-knowledge outcomes are the
        # source paper's own, and are about programme contact rather than health.
        "received_gmp_card": "service-utilisation outcome, not a health outcome",
        "heard_of_gmp": "service-utilisation outcome, not a health outcome",
        "can_explain_chart": "growth-chart knowledge, not a health outcome",
        "explains_chart_colours": "growth-chart knowledge, not a health outcome",
    },
    "nepal_hypertension": {
        # Still available as a predictor -- withdrawn only as an outcome.
        "bmi": "adjusted R-squared of 0.064; thirteen predictors explain almost none "
               "of the variation, and every surviving effect is under 1.4 kg/m2",
    },
}
...
def build_row(slug, number):
    ...
    return f'''      <tr>
        ...
        <td class="outcomes"><ul class="outcome-list">{lin_html}</ul></td>
        <td class="outcomes"><ul class="outcome-list">{log_html}</ul></td>
      </tr>'''
