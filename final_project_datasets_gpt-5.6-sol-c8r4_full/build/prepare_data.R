...
# ---------------------------------------------------------------------------
# 6. famuss_strength -- Thompson et al. MSSE 2004 (FAMuSS study)
# ---------------------------------------------------------------------------
d <- read_csv(file.path(CLEANED, "19_famuss.csv"), show_col_types = FALSE)
put("famuss_strength", tibble(
  ...
  bmi                 = d$bmi,
  actn3_genotype      = factor(d$actn3_genotype, levels = c("CC", "CT", "TT")),
  nondom_arm_change_pct = d$ndrm_change_pct,
  dom_arm_change_pct    = d$drm_change_pct
))
...
# ---------------------------------------------------------------------------
# 9. nhanes_adults -- NHANES 2009-2012 adult sample (OpenIntro 500-row extract)
#    Trimmed: all-NA child columns, second-void urine, and the sexual-behaviour
#    block are dropped. Everything kept is >= 82% complete except Testosterone,
#    which is retained because it is a genuinely interesting outcome.
# ---------------------------------------------------------------------------
d <- read_csv(file.path(CLEANED, "openintro_nhanes_samp_adult_500.csv"), show_col_types = FALSE)
d <- distinct(d, ID, .keep_all = TRUE)
yn <- function(v) as.integer(v == "Yes")
put("nhanes_adults", tibble(
  id                  = d$ID,
  survey_cycle        = factor(d$SurveyYr),
  ...
  marital_status      = factor(d$MaritalStatus),
  household_income_mid = d$HHIncomeMid,
  poverty_ratio       = d$Poverty,
  home_owner          = as.integer(d$HomeOwn == "Own"),
  work_status         = factor(d$Work),
  weight_kg           = d$Weight,
  height_cm           = d$Height,
  bmi                 = d$BMI,
  obese               = as.integer(d$BMI >= 30),
  pulse               = d$Pulse,
  systolic_bp         = d$BPSysAve,
  diastolic_bp        = d$BPDiaAve,
  hypertensive        = as.integer(d$BPSysAve >= 130 | d$BPDiaAve >= 80),
  total_cholesterol   = d$TotChol,
  hdl_cholesterol     = d$DirectChol,
  testosterone        = d$Testosterone,
  diabetes            = yn(d$Diabetes),
  general_health      = factor(d$HealthGen, levels = c("Poor", "Fair", "Good", "Vgood", "Excellent")),
  days_phys_health_bad = d$DaysPhysHlthBad,
  days_ment_health_bad = d$DaysMentHlthBad,
  sleep_hours         = d$SleepHrsNight,
  sleep_trouble       = yn(d$SleepTrouble),
  phys_active         = yn(d$PhysActive),
  alcohol_days_year   = d$AlcoholYear,
  smoked_100_cigs     = yn(d$Smoke100)
))

# ---------------------------------------------------------------------------
# 10. us_births_2014 -- NCHS 2014 natality random sample (openintro::births14)
# ---------------------------------------------------------------------------
d <- read_csv(file.path(CLEANED, "21_births14.csv"), show_col_types = FALSE)
put("us_births_2014", tibble(
  mother_age_yrs      = d$mother_age_yrs,
  father_age_yrs      = d$father_age_yrs,
  mature_mother       = as.integer(d$mother_maturity == "mature_mom"),
  married             = as.integer(d$marital_status == "married"),
  white_mother        = as.integer(d$white_mother),
  smoker              = as.integer(d$smoking_habit == "smoker"),
  weight_gain_lb      = d$weight_gain_lb,
  prenatal_visits     = d$prenatal_visits,
  gestation_weeks     = d$gestation_weeks,
  baby_sex            = factor(d$sex, levels = c("female", "male")),
  birthweight_lb      = d$birthweight_lb,
  # The source flag is inconsistent at the displayed boundary of 5.5 lb.
  # Rebuild it from the rounded birthweight so every 5.5 lb birth is treated alike.
  low_birthweight     = as.integer(d$birthweight_lb <= 5.5),
  premature           = as.integer(d$premature)
))
